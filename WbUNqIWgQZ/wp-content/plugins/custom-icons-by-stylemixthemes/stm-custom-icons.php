...
Version: 3.0.2
*/
define('STM_CUSTOM_ICONS_PATH', dirname(__FILE__));
define('STM_CUSTOM_ICONS_URL', plugin_dir_url(__FILE__));
...
add_action('elementor/preview/enqueue_styles', 'stm_custom_fonts');
add_action('elementor/editor/before_enqueue_styles', 'stm_custom_fonts');


//Custom icons
function stm_elementor_add_custom_icons( $controls_registry ) {
    // Get existing icons
    $default_icons = $controls_registry->get_control( 'icon' )->get_settings( 'options' );
    // Append new icons
    include(STM_CUSTOM_ICONS_PATH . '/assets/fonts/stm/charmap.php');
    $new_icons = array();
    if(!empty($icons['stm'])){
        foreach($icons['stm'] as $key => $value){
            $new_icons['stm-' . $key] = $key;
        }
        $default_icons = array_merge($new_icons, $default_icons);
    }

    // Then we set a new list of icons as the options of the icon control
    $controls_registry->get_control( 'icon' )->set_settings( 'options', $default_icons );
}
add_action( 'elementor/controls/controls_registered', 'stm_elementor_add_custom_icons', 10, 1 );

//Custom icons new
function consulting_elementor_add_custom_icons_new( $tabs = array() ) {

    $new_icons[ 'consulting-icons' ] = [
        'name'          => 'consulting-icons',
        'label'         => __( "Consulting icons", "stm_domain" ),
        'url'           => '',
        'enqueue'       => '',
        'prefix'        => '',
        'displayPrefix' => '',
        ...
        'ver'           => '1.0.1',
        'fetchJson'     => STM_CUSTOM_ICONS_URL . '/assets/fonts/stm/charmap.json',
    ];

    return array_merge( $tabs, $new_icons );
}
add_action( 'elementor/icons_manager/additional_tabs', 'consulting_elementor_add_custom_icons_new', 9999999, 1 );