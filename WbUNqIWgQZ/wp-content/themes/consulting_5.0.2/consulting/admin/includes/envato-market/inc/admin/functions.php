...
function envato_market_themes_column( $group = 'install' ) {
	...
	foreach ( $premium as $slug => $theme ) :
		...
		if ( true === $theme['hasUpdate'] ) {
			...
			if ( current_user_can( 'update_themes' ) ) {
				...
				$updateActions['update'] = sprintf(
					...
					esc_attr__( 'Update %s now', 'consulting' ),
					esc_attr( $name ),
					esc_attr( $slug ),
					esc_attr( $theme['version'] ),
					esc_html__( 'Update Available', 'consulting' )
				);
				...
				$updateActions['details'] = sprintf(
					...
					esc_attr( $name ),
					sprintf(
						__( 'View version %1$s details.', 'consulting' ),
						$theme['version']
					)
				);
			}
		...
		if ( 'active' === $group && current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
			...
			$actions['customize'] = '<a href="' . esc_url( $customize_url ) . '" class="button button-primary load-customize hide-if-no-customize"><span aria-hidden="true">' . __( 'Customize', 'consulting' ) . '</span><span class="screen-reader-text">' . sprintf( __( 'Customize &#8220;%s&#8221;', 'consulting' ), $name ) . '</span></a>';
			...
			if ( is_multisite() && ! $get_theme->is_allowed( 'both' ) && current_user_can( 'manage_sites' ) ) {
				...
				if ( current_user_can( 'manage_sites' ) ) {
					$actions['site_enable'] = '<a href="' . esc_url( network_admin_url( wp_nonce_url( 'site-themes.php?id=' . get_current_blog_id() . '&amp;action=enable&amp;theme=' . urlencode( $slug ), 'enable-theme_' . $slug ) ) ) . '" class="button"><span aria-hidden="true">' . __( 'Site Enable', 'consulting' ) . '</span><span class="screen-reader-text">' . sprintf( __( 'Site Enable &#8220;%s&#8221;', 'consulting' ), $name ) . '</span></a>';
				}

				if ( current_user_can( 'manage_network_themes' ) ) {
					$actions['network_enable'] = '<a href="' . esc_url( network_admin_url( wp_nonce_url( 'themes.php?action=enable&amp;theme=' . urlencode( $slug ) . '&amp;paged=1&amp;s', 'enable-theme_' . $slug ) ) ) . '" class="button"><span aria-hidden="true">' . __( 'Network Enable', 'consulting' ) . '</span><span class="screen-reader-text">' . sprintf( __( 'Network Enable &#8220;%s&#8221;', 'consulting' ), $name ) . '</span></a>';
				}
			}
			...
			if ( $can_activate && current_user_can( 'switch_themes' ) ) {
				...
				// Activate link.
				$actions['activate'] = '<a href="' . esc_url( $activate_link ) . '" class="button"><span aria-hidden="true">' . __( 'Activate', 'consulting' ) . '</span><span class="screen-reader-text">' . sprintf( __( 'Activate &#8220;%s&#8221;', 'consulting' ), $name ) . '</span></a>';
				...
				if ( current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
					...
					$actions['customize_preview'] = '<a href="' . esc_url( $preview_url ) . '" class="button button-primary load-customize hide-if-no-customize"><span aria-hidden="true">' . __( 'Live Preview', 'consulting' ) . '</span><span class="screen-reader-text">' . sprintf( __( 'Live Preview &#8220;%s&#8221;', 'consulting' ), $name ) . '</span></a>';
				}
			}
			...
			<a href="' . wp_nonce_url( $install_link, 'install-theme_' . $theme['id'] ) . '" class="button button-primary">
				<span aria-hidden="true">' . __( 'Install', 'consulting' ) . '</span>
				<span class="screen-reader-text">' . sprintf( __( 'Install %s', 'consulting' ), $name ) . '</span>
			</a>';
		}
				...
				<div class="envato-card-top">
					...
					<div class="column-name">
						<h4>
							...
							<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'consulting' ); ?>"><?php echo esc_html( sprintf( __( 'Version %s', 'consulting' ), $version ) ); ?></span>
						</h4>
					</div>
					<div class="column-description">
						...
						<p class="author">
							<cite><?php esc_html_e( 'By', 'consulting' ); ?> <?php echo wp_kses_post( $author_link ); ?></cite>
						</p>
					</div>
	...
	foreach ( $premium as $slug => $plugin ) :
		...
		if ( 'active' === $group ) {
			...
			<a href="' . wp_nonce_url( $deactivate_link, 'deactivate-plugin_' . $slug ) . '" class="button">
				<span aria-hidden="true">' . __( 'Deactivate', 'consulting' ) . '</span>
				<span class="screen-reader-text">' . sprintf( __( 'Deactivate %s', 'consulting' ), $name ) . '</span>
			</a>';
		} elseif ( 'installed' === $group ) {
			if ( ! is_multisite() && current_user_can( 'delete_plugins' ) ) {
				...
				<a href="' . wp_nonce_url( $delete_link, 'bulk-plugins' ) . '" class="button-delete">
					<span aria-hidden="true">' . __( 'Delete', 'consulting' ) . '</span>
					<span class="screen-reader-text">' . sprintf( __( 'Delete %s', 'consulting' ), $name ) . '</span>
				</a>';
			}
			...
			if ( ! is_multisite() && current_user_can( 'activate_plugins' ) ) {
				...
				<a href="' . wp_nonce_url( $activate_link, 'activate-plugin_' . $slug ) . '" class="button">
					<span aria-hidden="true">' . __( 'Activate', 'consulting' ) . '</span>
					<span class="screen-reader-text">' . sprintf( __( 'Activate %s', 'consulting' ), $name ) . '</span>
				</a>';
			}
			...
			if ( is_multisite() ) {
				if ( current_user_can( 'manage_sites' ) ) {
					...
					<a href="' . esc_url( admin_url( wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . urlencode( $slug ), 'activate-plugin_' . $slug ) ) ) . '" class="button">
						<span aria-hidden="true">' . __( 'Site Activate', 'consulting' ) . '</span>
						<span class="screen-reader-text">' . sprintf( __( 'Site Activate %s', 'consulting' ), $name ) . '</span>
					</a>';
				}
				...
				if ( current_user_can( 'manage_network_plugins' ) ) {
					...
					<a href="' . esc_url( network_admin_url( wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . urlencode( $slug ), 'activate-plugin_' . $slug ) ) ) . '" class="button">
						<span aria-hidden="true">' . __( 'Network Activate', 'consulting' ) . '</span>
						<span class="screen-reader-text">' . sprintf( __( 'Network Activate %s', 'consulting' ), $name ) . '</span>
					</a>';
				}
			}
			// @codeCoverageIgnoreEnd
			...
			<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
				<div class="envato-card-top">
					...
					<div class="column-name">
						<h4>
							<a href="<?php echo esc_url( $url ); ?>"><?php echo esc_html( $name ); ?></a>
							<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'consulting' ); ?>"><?php echo esc_html( sprintf( __( 'Version %s', 'consulting' ), ( isset( $plugins[ $slug ] ) ? $plugins[ $slug ]['Version'] : $version ) ) ); ?></span>
						</h4>
					</div>
					<div class="column-description">
						<div class="description">
