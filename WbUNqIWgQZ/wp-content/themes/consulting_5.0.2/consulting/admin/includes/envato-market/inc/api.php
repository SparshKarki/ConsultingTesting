...
if ( ! class_exists( 'Envato_Market_API' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_API {
		...
		public function request( $url, $args = array() ) {
			...
			if ( empty( $token ) ) {
				return new WP_Error( 'api_token_error', __( 'An API token is required.', 'consulting' ) );
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				...
				return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'consulting' ) );
			} else {
				$return = json_decode( wp_remote_retrieve_body( $response ), true );
				if ( null === $return ) {
					return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'consulting' ) );
				}
				return $return;
			}
		}
