...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'consulting' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
		 			wp_die( __( 'You do not have sufficient permissions to delete transients.', 'consulting' ) );
				}
		...
		public function add_menu_page() {
			$page = add_menu_page( __( 'Envato Market', 'consulting' ), __( 'Envato Market', 'consulting' ), 'manage_options', envato_market()->get_slug(), array( $this, 'render_admin_callback' ) );
		...
		public function admin_enqueue_script() {
			...
			$exports = array(
				...
				'i18n' => array(
					'save' => __( 'Save', 'consulting' ),
					'remove' => __( 'Remove', 'consulting' ),
					'cancel' => __( 'Cancel', 'consulting' ),
					'error' => __( 'An unknown error occurred. Try again.', 'consulting' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'consulting' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'consulting' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'consulting' ) ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_attr_e( 'Remove Item', 'consulting' ) ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'consulting' ) ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_attr_e( 'Add Item', 'consulting' ) ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'consulting' ) ?></label>
							<input type="text" name="token" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'consulting' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'consulting' ) ?></label>
							<input type="text" name="id" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'consulting' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'consulting' ); ?>"><?php esc_html_e( 'Version', 'consulting' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'consulting' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'consulting' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'consulting' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				__( 'Global OAuth Personal Token', 'consulting' ),
			...
			add_settings_section(
				...
				__( 'Single Use OAuth Personal Tokens', 'consulting' ),
			...
			add_settings_field(
				'items',
				__( 'Envato Market Items', 'consulting' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'consulting' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install plugins on this site.', 'consulting' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'consulting' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'consulting' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'consulting' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'consulting' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title = sprintf( __( 'Installing Plugin: %s', 'consulting' ), $api->name . ' ' . $api->version );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'consulting' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install themes on this site.', 'consulting' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Theme Installer', 'consulting' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'consulting' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'consulting' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Plugin Installer', 'consulting' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title = sprintf( __( 'Installing Theme: %s', 'consulting' ), $api->name . ' ' . $api->version );
		...
		public function ajax_upgrade_theme() {
			...
			if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
				$status['oldVersion'] = sprintf( __( 'Version %s', 'consulting' ), $theme_data->get( 'Version' ) );
			}

			if ( ! current_user_can( 'update_themes' ) ) {
				$status['error'] = __( 'You do not have sufficient permissions to update themes for this site.', 'consulting' );
			...
			if ( is_array( $result ) && ! empty( $result[ $theme ] ) ) {
				...
				if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
					$status['newVersion'] = sprintf( __( 'Version %s', 'consulting' ), $theme_data->get( 'Version' ) );
				}
				...
				$status['error'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'consulting' );
		...
		public function ajax_add_item() {
			if ( ! check_ajax_referer( self::AJAX_ACTION, 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'The Token is missing.', 'consulting' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => __( 'The Item ID is missing.', 'consulting' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => __( 'The Token or Item ID is incorrect.', 'consulting' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => __( 'The item cannot be downloaded.', 'consulting' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An unknown error occurred.', 'consulting' ) ) );
		}
